...
# class DataDome:
#     def __init__(self, ddv="5.1.5", ddk="AE3F04AD3F0D3A462481A337485081"):
#         self.__ddv = ddv
#         self.__ddk = ddk
...
#     def build(self, url):
#     #     stcfp = f"""ps://dd.garena.com/js/?id={self.__ddk}:2:90854)
#     # at https://dd.garena.com/js/?id={self.__ddk}:2:53225"""
#         stcfp = f"""ps://js.datadome.co/tags.js?id={self.__ddk}:2:90854)
#     at https://js.datadome.co/tags.js?id={self.__ddk}:2:53225"""
#     #     stcfp = f"""ps://dd.garena.com/tags.js?id={self.__ddk}:2:90854)
#     # at https://dd.garena.com/tags.js?id={self.__ddk}:2:53225"""
#         # stcfp = f"https://dd.garena.com/tags.js?id={self.__ddk}"
...
# #     dd = DataDome()
# #     data = dd.build("https://sso.garena.com/")
    
# #     # Gọi API DataDome qua domain Garena
# #     url = "https://dd.garena.com/js/"
# #     r = requests.post(url, headers=data["headers"], data=data["payload"])
# #     print("Response:", r.text)

# #     # Parse cookie datadome từ response JSON
# #     try:
# #         datadome_cookie = r.json()["cookie"].split("datadome=")[1].split(";")[0]
# #         print("✅ Datadome:", datadome_cookie)
# #     except:
# #         print("❌ Không lấy được datadome cookie!")
import requests
import random
import time
import base64,json
...
class DataDome:
    def __init__(self, ddv="5.1.5", ddk="AE3F04AD3F0D3A462481A337485081"):
        ...
        self.__ddk = ddk
        self.__screen = Screen(
            min_width=1280, max_width=5120,
            min_height=720, max_height=2880
        )
        self.__headers = FingerprintGenerator(
            browser=("chrome", "firefox", "safari", "edge"),
            os=("windows", "macos"),
    ...
    def build(self, referer_url):
        """Trả về headers + payload để POST lên dd.garena.com/js/"""
        stcfp = f"ps://dd.garena.com/js/?id={self.__ddk}:2:90854)"
        payload = {
            "ddv": self.__ddv,
            "eventCounters": {
                # fake vài events để DataDome tin
                "mousemove": random.randint(100, 300),
                "click": random.randint(1, 6),
                "scroll": random.randint(0, 20),
                "keydown": random.randint(0, 5),
                "keyup": random.randint(0, 5)
            },
            "jsType": "ch",
            "ddk": self.__ddk,
            "request": "%2F",               # tùy bạn set đúng endpoint encode
            "responsePage": "origin",
            "cid": "null",
            "Referer": referer_url,
            "jsData": {
                "ttst": f"{random.randint(10,99)}.{random.randint(10**12,10**13)}",
                "ifov": "false",
                "hc": self.__temps.navigator.hardwareConcurrency,
                "br_oh": self.__temps.screen.outerHeight,
                "br_ow": self.__temps.screen.outerWidth,
                "ua": self.__temps.headers.get("User-Agent"),
                # … bạn có thể thêm bớt trường jsData tuỳ ý …
                "jset": int(time.time())
            }
        }
        # stringify eventCounters and jsData
        payload["eventCounters"] = json.dumps(payload["eventCounters"])
        payload["jsData"]       = json.dumps(payload["jsData"])
        headers = {
            "Content-Type":    "application/x-www-form-urlencoded",
            "Host":            "dd.garena.com",
            "Origin":          referer_url,
